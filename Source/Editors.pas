...
begin
  ...
  else if FEdit is TRzColorEdit then
    S := IntToHex(RGBandBGR(TRzColorEdit(FEdit).SelectedColor), 6)
  else
  begin
  ...
  case Data.DataType of
    jdtString:
      begin
        ...
        if FColumn = 2 then
        begin
          FEdit := TRzColorEdit.Create(nil);
          with FEdit as TRzColorEdit do
          begin
            BorderStyle:= bsNone;
            ...
            Parent := Tree;
            //Text := Data.Value;
            SelectedColor := RGBandBGR(StrToInt('$' + Data.Value));
            OnKeyDown := EditKeyDown;
            OnKeyUp := EditKeyUp;
          end;
        end;
